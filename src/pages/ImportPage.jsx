...
const MetaChip = styled.span`
  font-family: var(--font-mono);
  background: ${p => (p.$suggested ? 'transparent' : p.$getty ? 'var(--getty-wash)' : 'var(--surface)')};
  color: ${p => (p.$getty ? 'var(--getty)' : 'var(--ink-soft)')};
  border: 1px ${p => (p.$suggested ? 'dashed' : 'solid')} ${p => (p.$getty ? 'var(--getty-line)' : 'var(--rule-strong)')};
  opacity: ${p => (p.$suggested ? 0.85 : 1)};
  padding: 3px 7px;
  border-radius: 999px;
  font-size: 12px;
  line-height: 1.15;
  ...
  cursor: pointer;
  user-select: none;
  &:hover { border-color: var(--danger); color: var(--danger); background: var(--danger-wash); opacity: 1; }
`;
...
export default function ImportPage() {
  ...
  const analyzeRow = async (row, extraPrompt = "") => {
    ...
    try {
      ...
      let nextGettySuggested = Array.isArray(row.gettyKeywordsSuggested) ? row.gettyKeywordsSuggested : [];
      ...
      if (data) {
        try {
          ...
          if (Array.isArray(payload.gettyKeywordsSuggested)) nextGettySuggested = payload.gettyKeywordsSuggested;
        ...
        prev.map((r) =>
          r.id === row.id
            ? {
                ...
                gettyKeywordsSuggested: nextGettySuggested,
            ...
            setRows(prev => {
              ...
              return prev.map(r => {
                ...
                return {
                  ...
                  gettyKeywordsSuggested: Array.isArray(fresh.gettyKeywordsSuggested) ? fresh.gettyKeywordsSuggested : (r.gettyKeywordsSuggested || []),
  ...
  const cols = [
    ...
    },
    {
      ...
      renderCell: ({ row, onRowChange }) => {
        ...
        const customList = asList(row.keywords);
        const gettyList = asList(row.gettyKeywords);
        const suggestedList = asList(row.gettyKeywordsSuggested);
        const isGettyDisplay = gettyList.length > 0 || suggestedList.length > 0;
        const chipsRef = React.useRef(null);
        const [hasDraft, setHasDraft] = useState(false);
        const addKeyword = (val) => {
          if (isGettyDisplay) return; // Getty output is generated by AI; keep add-only for custom keywords.
          const t = (val || '').trim();
          if (!t) return;
        ...
        const removeCustomAt = (idx) => {
          const next = customList.filter((_, i) => i !== idx);
          onRowChange({ ...row, keywords: next }, true);
          saveMetadataChanges(row.id, { keywords: next });
          showToast('Keywords updated');
        };
        const removeGetty = (tier, idx) => {
          const nextCore = tier === 'core' ? gettyList.filter((_, i) => i !== idx) : gettyList;
          const nextSug = tier === 'suggested' ? suggestedList.filter((_, i) => i !== idx) : suggestedList;
          onRowChange({ ...row, gettyKeywords: nextCore, gettyKeywordsSuggested: nextSug }, true);
          saveMetadataChanges(row.id, { gettyKeywords: nextCore, gettyKeywordsSuggested: nextSug });
          showToast('Getty keywords updated');
        };
        const handleKeyDown = (e) => {
          ...
          if (e.key === 'Backspace') {
            ...
            if (!content && !isGettyDisplay && customList.length) {
              e.preventDefault();
              removeCustomAt(customList.length - 1);
            }
          }
        };
        return (
          <MetaChipsWrapper>
            ...
            >
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {isGettyDisplay ? (
                  <>
                    {gettyList.map((kw, idx) => (
                      <MetaChip key={`c${idx}`} $getty title="Click to remove" onClick={() => removeGetty('core', idx)}>{kw}</MetaChip>
                    ))}
                    {suggestedList.map((kw, idx) => (
                      <MetaChip key={`s${idx}`} $getty $suggested title="Suggested — click to remove" onClick={() => removeGetty('suggested', idx)}>{kw}</MetaChip>
                    ))}
                  </>
                ) : (
                  customList.map((kw, idx) => (
                    <MetaChip key={idx} title="Click to remove" onClick={() => removeCustomAt(idx)}>{kw}</MetaChip>
                  ))
                )}
                <MetaEditableKeywords
                  ref={chipsRef}
              ...
              {!isGettyDisplay && !customList.length && !hasDraft && (
                <MetaPlaceholder>Type keyword and press Enter</MetaPlaceholder>
              )}
            </MetaChips>
            <KeywordCountBadge title={suggestedList.length ? `${gettyList.length} verified + ${suggestedList.length} suggested` : undefined}>
              {isGettyDisplay
                ? (suggestedList.length ? `${gettyList.length} +${suggestedList.length}` : gettyList.length)
                : customList.length}
            </KeywordCountBadge>
          </MetaChipsWrapper>
        );
  ...
  const exportWindowsCsv = async (shootDateOverride = '', countryOverride = '') => {
    ...
    try {
      ...
      for (const r of rows) {
        ...
        // Export both tiers — the photographer has already pruned anything wrong in the grid.
        const gettyCoreArr = Array.isArray(r.gettyKeywords) ? r.gettyKeywords : [];
        const gettySuggestedArr = Array.isArray(r.gettyKeywordsSuggested) ? r.gettyKeywordsSuggested : [];
        const seenGetty = new Set(gettyCoreArr.map(s => String(s).toLowerCase()));
        const gettyKeywordsArr = [...gettyCoreArr, ...gettySuggestedArr.filter(s => !seenGetty.has(String(s).toLowerCase()))];
