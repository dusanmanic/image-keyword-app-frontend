/**
 * Image Analysis API Service
 */

import { getAuthHeaders } from "./authService.js";
import { getApiBaseUrl } from "../config/api.js";

/**
 * Analyze image and generate metadata (AI generates custom keywords, mapping to Getty happens during export)
 * @param {Blob} imageBlob - Image blob to analyze
 * @param {number} maxKeywords - Maximum number of keywords to generate
 * @param {string} [prompt] - Optional additional prompt
 * @param {{ gettyMode?: "max" | "medium" | "low" | "lowest" }} [options] - Getty generation mode
 * @returns {Promise<{title: string, description: string, keywords: string[], gettyKeywords?: string[]}>}
 */
export async function analyzeImage(imageBlob, maxKeywords = 30, prompt = "", options = {}) {
  const formData = new FormData();
  formData.append("image", imageBlob);
  formData.append("maxKeywords", String(maxKeywords));
  
  if (prompt && prompt.trim()) {
    formData.append("prompt", prompt.trim());
  }
  if (options?.gettyMode) {
    formData.append("gettyMode", String(options.gettyMode));
  }

  const headers = getAuthHeaders();

  const res = await fetch(`${getApiBaseUrl()}/api/analyze/analyze`, {
    method: "POST",
    headers,
    body: formData,
  });

  if (!res.ok) {
    throw new Error(`Analysis failed: ${res.status}`);
  }

  return await res.json();
}
